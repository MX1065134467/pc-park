    ...
    <el-dialog title="查看" :visible.sync="checkDialog" class="check_dialog" :close-on-click-modal="false">
      <ul class="check_content">
        <li>
          ...
          <span>申请时间：{{checkData.createTime}}</span>
        </li>
        <li>
        ...
        <li>
          <span>申报渠道：{{checkData.channelTypeStr}}</span>
          <span>申报账户：{{checkData.hotelName}}</span>
          <span>状态：{{checkData.statusStr}}</span>
        </li>
        <li><span>有效时间：{{checkData.beginTime}} 至 {{checkData.endTime}}</span></li>
        <li>
...
import { downloadFile, timeFormat } from '@/utils/util.js'
export default {
  name: 'white',
  ...
  methods: {
    ...
    async getList() {
      ...
      const { data: res } = await serve.getHoteList(params)
      this.loading = true
      if (res.code === 200) {
        this.loading = false
    ...
    exportData() {
      ...
      serve.exportParkCore(params).then(res => {
        console.log(res, 5522)
        if (res.status === 200) {
          downloadFile(res.data, '酒店合作异业名单')
        }
      })
    },
    ...
    deleteData() {
      ...
      this.multipleSelection.map(v => {
        removeArr.push(v.vipId)
      })
      this.modalMessage.title = '批量移除'
      this.modalMessage.show = true
      this.modalMessage.content = '是否从酒店异业合作中移除该车主？'
    ...
    handleDialog(val) {
      console.log(val)
      const params = val.removeList || val.vipId
      serve.updataHotelStatus(params).then(res => {
    ...
    stopOrStartUse(row, type) {
      ...
      switch (type) {
        case '查看':
          serve.getHotelDetail(vipId).then(res => {
            if (res.data.code === 200) {
        ...
        case '移除':
          this.modalMessage.title = type
          this.modalMessage.show = true
          this.modalMessage.content = '是否从酒店异业中移除该车主？'
