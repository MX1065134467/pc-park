  ...
  <div class="ticket-wrap container" v-loading="loading" element-loading-text="加载中"
    ...
    <div class="content">
      <div class="content_grid">
        ...
        <el-row :gutter="20" class="first_grid img_row" v-show="isList">
        ...
        <el-row :gutter="20" class="first_grid">
        <el-col :span="6"><span>停车时长：</span>{{carInOut.parkHour}} 小时</el-col>
        <el-col :span="6"><span>停车费用（￥）：</span>{{orderInfo.billingAmount}} 元</el-col>
        </el-row>
      </div>
      <div class="content_grid">
        <h3>订单信息</h3>
        <el-row :gutter="20" class="first_grid">
          ...
          <el-col :span="6"><span>停车类型：</span>{{orderInfo.showParkType}}</el-col>
...
export default {
  ...
  methods: {
    async init () {
      ...
      if (res.code === 200) {
        console.log(res.data.carInOut)
        this.loading = false
        this.allData = res.data
        this.carInOut = res.data.carInOut
        this.orderInfo = res.data.orderInfo
        this.orderPay = res.data.orderPay
        this.imgList.push(this.carInOut.inImages[0])
        this.outImgList.push(this.carInOut.outImages[0])
        res.data.carInOut.snapshotList.map(item => {
          if (item === null) {
            this.isList = false
          } else {
            this.isList = true
